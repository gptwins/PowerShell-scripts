<#
    .SYNOPSIS
    A quick PowerShell script to return the Trend Vision One Standard Endpoint Protection/Apex One agent install directory, server name/address, policy name applied, and the last time the policy was applied.

    .DESCRIPTION
    The script reads the Windows registry to determine of the Vision One Standard Endpoint Protection/Apex One agent is installed.  It returns the installation directory, the fully qualified server name or IP address of the server, The SEP/Apex One agent policy that is currently applied, and the last date/time the policy was applied to the agent.

    .PARAMETER Version
    There is only one available switch, that is to show the current version number of the script.

    .EXAMPLE
    PS C:\Users\gdgee\src\ps1> .\Get-SEP-PolicyInfo.ps1
